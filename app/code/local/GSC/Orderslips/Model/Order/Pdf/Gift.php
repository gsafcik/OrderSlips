<?php
/**
 * Gift Receipt Sales PDF abstract model
 *
 * @package    Mage_Sales (original)
 * @author     GSC <gsafcik@yahoo.com>
 */
abstract class GSC_Orderslips_Model_Order_Pdf_Gift extends Varien_Object
{
    public $y;
    /**
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Gift Receipt for Order #').$order->getRealOrderId(), 35, 720, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 35, 760, 'UTF-8'); // DEFAULT
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
            $page->drawText(Mage::helper('sales')->__('Shipping Method:'), 35, $this->y , 'UTF-8');
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->drawText($shippingMethod, 35, $this->y, 'UTF-8');
            ...
            if (count($tracks)) {
                $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
                $page->setLineWidth(0.5);
                $page->drawRectangle(35, $yShipments-5, 280, $yShipments - 15);
                $page->drawLine(157, $yShipments-5, 157, $yShipments - 15);

                $this->_setFontRegular($page);
                $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
                $page->drawText(Mage::helper('sales')->__('Title'), 40, $yShipments - 13, 'UTF-8');
                $page->drawText(Mage::helper('sales')->__(' Tracking Number'), 162, $yShipments - 13, 'UTF-8');
                ...
                foreach ($tracks as $track) {
                    ...
                    $page->drawText($truncatedTitle, 40, $yShipments-7, 'UTF-8');
                    $page->drawText($track->getNumber(), 162, $yShipments-7, 'UTF-8');
                    $yShipments -=7;
                }
            } else {
                $yShipments -= 7;
            ...
            $currentY = min($yPayments, $yShipments)-10;

            // replacement of Shipments-Payments rectangle block
            $page->drawLine(25, $this->y + 15, 25, $currentY); // left
            $page->drawLine(25, $currentY, 570, $currentY); // bottom
            $page->drawLine(570, $currentY, 570, $this->y + 15); //right
    ...
    //protected function _setFontBold($object, $size = 7) // DEFAULT
